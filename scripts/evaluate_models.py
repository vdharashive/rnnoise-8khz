...
def export_model_to_c(model_path, export_dir, model_name):
    ...
    try:
        ...
        cmd = [
            sys.executable,
            os.path.join(os.path.dirname(__file__), '..', 'torch', 'rnnoise', 'dump_rnnoise_weights.py'),
            '--quantize',
            model_path,
            model_export_dir
        ]
        ...
        result = subprocess.run(cmd, capture_output=True, text=True, cwd=os.path.dirname(cmd[1]))
    ...
    try:
        ...
        for filename in ['rnnoise_data.c', 'rnnoise_data.h']:
            ...
            if os.path.exists(exported_file):
                shutil.copy2(exported_file, os.path.join(src_dir, filename))

        # Recompile
        print(f"Compiling RNNoise with {model_name}...")
        cmd = ['make', 'clean']
        subprocess.run(cmd, cwd=rnnoise_root, check=True)
...
def calculate_audio_metrics(original, processed):
    ...
    if processed is None or len(processed) != len(original):
        return None
    ...
    noise_original = original - processed  # Assuming processed is "clean" version
    snr_original = 10 * np.log10(np.mean(original**2) / (np.mean(noise_original**2) + 1e-10))
    snr_processed = 10 * np.log10(np.mean(processed**2) / (np.mean(noise_original**2) + 1e-10))
